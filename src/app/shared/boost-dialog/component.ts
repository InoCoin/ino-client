...
export class BoostDialog implements OnInit {
  ...
  ngOnInit() {
    this.AccountProvider.getAllUserAccounts().subscribe((res: any) => {
      if (res.result) {
  ...
  onSubmit() {
    ...
    this.Web3Provider.getPrivKey(acc.keyStore, this.boostForm.controls.password.value).then((accountData: any) => {
      ...
      this.Web3Provider.createTransaction(accountData.privateKey, acc.address, Number(this.boostForm.controls.amount.value), this.boostForm.controls.address.value).then((transactionHash) => {
