...
import { ApiProvider, Web3Provider, LoaderProvider, PaymentProvider, MapProvider } from '../../providers';
      ...
      case (PaymentMethods.ether.key): {
        if (this.formETH.valid && this.formETH.controls.amount.value > 0) {
          ...
          this.Web3Provider.getPrivKey(this.data.account.keyStore, this.formETH.controls.password.value).then((privKey: any) => {
            ...
            this.Web3Provider.sendEthers(privKey.privateKey, this.data.account.address, this.total, Account).then((transactionHash) => {
